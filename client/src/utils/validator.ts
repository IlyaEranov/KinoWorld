enum ValidatorErrors{
    ...
    isPassword = "Пароль должен содеражать не менее 8 символов и включать как минимум 1 цифру, 1 заглавную и одну строчную латинские буквы, а также 1 спец. символ",
    isName = "Имя пользователя должно быть не меньше 3 и не больше 16 символов",
...
const validate = (key: string, value: any) => {
    ...
    switch(key){
        ...
        case "name":
            return value.length <= 16 && value.length >= 3 ? null : ValidatorErrors.isName
        default:
            return null
    }
}
